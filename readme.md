...
### Task 1: Write Code
1. Open `src/AdvancedOperations.mjs` and implement the methods using GitHub Copilot assistance:
    - `findMax`
    - `reverseString`
    - `formatDate`
    - `factorial`
...
### Task 2: Write Unit Tests
    ...
    You might consider approaching the task using Test-Driven Development (TDD), while following the basic flow outlined in the instructions.
...
2. Open `test/test.mjs` and write unit tests for methods above, making sure the coverage is acceptable.
...
1. Review the `calculateStatistics` method in `src/AdvancedOperations.mjs` that calculates statistics for a set of numbers but utilizes bad practices and violates DRY, KISS, SOLID, and other principles.

2. Use GitHub Copilot's refactor feature to refactor the `calculateStatistics` method to follow DRY, KISS, SOLID, and other principles.
